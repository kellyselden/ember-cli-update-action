...
const shebangCommand = __webpack_require__(9152);
...
const path = __webpack_require__(6928);
const which = __webpack_require__(6848);
const getPathKey = __webpack_require__(6689);
...
const shebangRegex = __webpack_require__(7334);
...
const isexe = __webpack_require__(2940)
...
const uint8ArraysToStrings = (uint8ArraysOrStrings, encoding) => {
	...
	const decoder = new external_node_string_decoder_.StringDecoder(encoding);
...
const hasColors = external_node_tty_?.WriteStream?.prototype?.hasColors?.() ?? false;
...
var cross_spawn = __webpack_require__(546);
...
const execFileOriginal = (0,external_node_util_.promisify)(external_node_child_process_.execFile);
...
const getSignalsIntegerToName = () => new Map(Object.entries(external_node_os_.constants.signals)
	.reverse()
...
const getAvailableSignalNames = () => Object.keys(external_node_os_.constants.signals)
	.sort()
	.map(signalName => `'${signalName}'`)
	.join(', ');

const getAvailableSignalIntegers = () => [...new Set(Object.values(external_node_os_.constants.signals)
...
const getEncodingTransformGenerator = (binary, encoding, skipped) => {
	...
	const stringDecoder = new external_node_string_decoder_.StringDecoder(encoding);
	return {
...
const handleEarlyError = ({error, command, escapedCommand, fileDescriptors, options, startTime, verboseInfo}) => {
	...
	const subprocess = new external_node_child_process_.ChildProcess();
